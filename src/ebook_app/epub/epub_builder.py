# src/ebook_app/pipeline/epub_builder.py
...
class EPUBBuilder:
    ...
    Controller contract:
        add_chapter(filename: str, xhtml: str, title: str) -> None
        add_audio(chapter_filename: str, audio_path: str, segments: List[Dict]) -> None
        build() -> Path

    Where segments are dicts:
        {
            "paragraph_id": str,
    ...
    def build(self) -> Path:
        ...
        safe_title = self._safe_filename(self.title or "book")
        out_path = self.output_dir / f"{safe_title}.epub"
    ...
    def _write_smil_for_chapter(self, chapter_filename: str) -> None:
        ...
        for seg in segments:
            ...
            ET.SubElement(
                par,
                "text",
                src=f"text/{chapter_filename}#{pid}",
            )

            clip_begin = self._clock(seg.get("clip_begin", 0.0))
            clip_end = self._clock(seg.get("clip_end", 0.0))
    ...
    def _write_nav(self) -> None:
        ...
        items = "\n".join(
            f'<li><a href="text/{c["filename"]}">{esc(c["title"])}</a></li>'

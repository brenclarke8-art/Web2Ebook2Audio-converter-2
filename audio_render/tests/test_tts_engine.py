...
def test_generate_audio_retries_once_on_503_and_succeeds(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("audio_render.tts_service.requests.post", _fake_post)
    monkeypatch.setattr("audio_render.tts_service.time.sleep", lambda *_: None)
...
def test_generate_audio_raises_after_retry_budget_exhausted(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("audio_render.tts_service.requests.post", _always_503)
    monkeypatch.setattr("audio_render.tts_service.time.sleep", lambda *_: None)
...
def test_generate_audio_does_not_retry_non_retryable_503(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("audio_render.tts_service.requests.post", _always_missing_model)
    monkeypatch.setattr("audio_render.tts_service.time.sleep", lambda *_: None)

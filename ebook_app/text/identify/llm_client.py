...
class OllamaChatClient:
    ...
    def __init__(
        ...
        *,
        ...
        # Map old args into LLMClient-compatible constructor
        provider = "ollama_local"
        # Use the same base_url and model; pass llm_log_path through
        ...
        # Keep a reference to the log path for external access (some callers check this)
        self.llm_log_path = llm_log_path

    def ask_json_any(self, *, system: str, user: str, chapter_id: str) -> Any:
        """
        Old callers expect ask_json_any(system=..., user=..., chapter_id=...)
        Return parsed JSON (dict or list) or raise on final failure (keeps old behavior).
        """
        # Delegate to generate_json; include chapter_id in logs via client's llm_log_path
        try:
            return self._client.generate_json(system=system, user=user)
        except Exception:
            # Preserve old behavior: re-raise so callers can catch if they expect exceptions
            raise

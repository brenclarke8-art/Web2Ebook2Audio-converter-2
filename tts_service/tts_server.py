# src/ebook_app/pipeline/tts_engine.py
...
class TTSEngineContract(Protocol):
    """
    Contract for all TTS engines used by the pipeline.

    Required methods:
        - generate_audio(text, output_filename, voice, speed)
...
# ----------------------------------------------------------------------
# Kokoro FastAPI Implementation
# ----------------------------------------------------------------------

class TTSEngine(TTSEngineContract):
    """
        ...
        *,
        ...
        for attempt in range(1, self.retry_attempts + 1):
            try:
                ...
                # Retry only if 503 AND retryable
    ...
    def concatenate_audio_files(
        ...
        for f in files:
            ...
            if sample_rate is None:
                ...
                raise RuntimeError(
                    f"Sample rate mismatch: {f} has {sr}, expected {sample_rate}"
                )
